describe "A - Vendor::Product" do
  ...
  describe "B - intialize" do 
    ...
    it "should raise error if no id param" do
      lambda{Vendor::Product.new}.should.raise(ArgumentError).message.should.match(/VENDOR WARNING: You forgot to write in your items id. You can't sell items without an id./)
    end

    it "should raise error if subscription param, but no secret param" do
      lambda{Vendor::Product.new(:id => "subscription_id", :subscription => true)}.should.raise(ArgumentError).message.should.match(/VENDOR WARNING: You're missing the shared secret./)
    end
